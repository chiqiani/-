...
#include<math.h>
int main(){
	cv::Mat src=cv::imread("/home/dqs/桌面/实验/4.jpeg");
    cv::imshow("1",src);
    cv::Mat src1,dst,labels,dst1,stats,centroids;
    cv::cvtColor(src,src1,cv::COLOR_BGR2GRAY);//将源图转化为单通道图像
    cv::threshold(src1,dst1,127,255,cv::THRESH_BINARY_INV);//二值化
    cv::imshow("2",dst1);
    cv::Mat element = getStructuringElement(cv::MORPH_RECT,cv::Size(2,2));//获取自定义核
    cv::morphologyEx(dst1,dst,cv::MORPH_OPEN,element);//闭运算
    cv::imshow("dst0",dst);
    int nccomps=cv::connectedComponentsWithStats(dst,labels,stats,centroids);//标记联通域
    std::cout << "回形针的个数: " << nccomps-1 << std::endl;//输出联通域个数
